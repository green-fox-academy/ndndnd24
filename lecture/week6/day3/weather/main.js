...
app.get('/', (req, res) => {
    let currentCity = []
    if (!req.query.city) {
        currentCity = forecasts;
    } else {
        // currentCity = forecasts.filter(item => item.weather.indexOf(req.query.city) >= 0);
        let x = cityList.indexOf(req.query.city)
        currentCity = forecasts[x];
    }
    res.render('home', {
...
const cityList = ['Seattle', 'Miami', 'Barcelona', 'London', 'Budapest'];
